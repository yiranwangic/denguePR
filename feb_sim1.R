...
iv_s1 <- (e1[361816,1]) + (i1[361816,1]) + (e2[361816,1]) + (i2[361816,1])
iv_s2 <- (e1[361816,2]) + (i1[361816,2]) + (e2[361816,2]) + (i2[361816,2])
iv_s3 <- (e1[361816,3]) + (i1[361816,3]) + (e2[361816,3]) + (i2[361816,3])
iv_s4 <- (e1[361816,4]) + (i1[361816,4]) + (e2[361816,4]) + (i2[361816,4])
iv1_t <- sum(e1[361816,]) + sum(i1[361816,])
iv2_t <- sum(e2[361816,]) + sum(i2[361816,])
iv_t <- iv1_t + iv2_t
pv1 <- iv1_t/iv_t

pv1_s1 <- (e1[361816,1] + i1[361816,1])/iv_s1
pv1_s2 <- (e1[361816,2] + i1[361816,2])/iv_s2
pv1_s3 <- (e1[361816,3] + i1[361816,3])/iv_s3
pv1_s4 <- (e1[361816,4] + i1[361816,4])/iv_s4
...
moP[1,] <- imop 
moS[1,] <- imos
tot <- su[1] + r[1] + v_s1[1] + v_s2[1] + v_s3[1] + v_s4[1] + sum(moP[1,]) + sum(moS[1,])
tot

# approximated initial values for E and I generated by using the quasi-equillibrium assumption and pSERO
e1[1,1] <- v_s1[1]*pv1_s1*(sigma/(sigma+omega))
i1[1,1] <- v_s1[1]*pv1_s1*(omega/(sigma+omega))

e1[1,2] <- v_s2[1]*pv1_s2*(sigma/(sigma+omega))
i1[1,2] <- v_s2[1]*pv1_s2*(omega/(sigma+omega))

e1[1,3] <- v_s3[1]*pv1_s3*(sigma/(sigma+omega))
i1[1,3] <- v_s3[1]*pv1_s3*(omega/(sigma+omega))

e1[1,4] <- v_s4[1]*pv1_s4*(sigma/(sigma+omega))
i1[1,4] <- v_s4[1]*pv1_s4*(omega/(sigma+omega))

e2[1,1] <- v_s1[1]*(1-pv1_s1)*(sigma/(sigma+omega))
i2[1,1] <- v_s1[1]*(1-pv1_s1)*(omega/(sigma+omega))

e2[1,2] <- v_s2[1]*(1-pv1_s2)*(sigma/(sigma+omega))
i2[1,2] <- v_s2[1]*(1-pv1_s2)*(omega/(sigma+omega))

e2[1,3] <- v_s3[1]*(1-pv1_s3)*(sigma/(sigma+omega))
i2[1,3] <- v_s3[1]*(1-pv1_s3)*(omega/(sigma+omega))

e2[1,4] <- v_s4[1]*(1-pv1_s4)*(sigma/(sigma+omega))
i2[1,4] <- v_s4[1]*(1-pv1_s4)*(omega/(sigma+omega))
...
w1 <- i1/e1
matplot(w1, type='l', lty=1,col=col_set,xaxt='n',main='')
matplot(w1[-(1:28),], type='l', lty=1,col=col_set,xaxt='n',main='', ylim = c(0.5, 0.8))

w2 <- i2/e2
matplot(w2, type='l', lty=1,col=col_set,xaxt='n',main='')
matplot(w2[-(1:28),], type='l', lty=1,col=col_set,xaxt='n',main='', ylim = c(0.5, 0.8))
...
data$Ncases <- cases[-(1:4),]
data$NcasesTot <- casetot[-(1:4)]
...
data$Tmax <- Tmax
data$m <- m
data$alpha <- c(20,60,rep(0.2,4),rep(5,8))/5

# fit the model to simulated data 
alpha <- c(20,60,rep(0.2,4),rep(5,8))/5
initI <- rdirichlet(100,alpha)
ii1 <- as.vector(initI[1,])
ii2 <- as.vector(initI[2,])
ii3 <- as.vector(initI[3,])
inits_chain1 <- list(init=ii1, log_a=-6.5, Tmin=17, Tmax=35, pv1_s1=0.3, pv1_s2=0.4, pv1_s3=0.5, pv1_s4=0.6)  
inits_chain2 <- list(init=ii2, log_a=-6.4, Tmin=16, Tmax=34, pv1_s1=0.4, pv1_s2=0.5, pv1_s3=0.6, pv1_s4=0.3)
inits_chain3 <- list(init=ii3, log_a=-6.6, Tmin=18, Tmax=36, pv1_s1=0.5, pv1_s2=0.6, pv1_s3=0.3, pv1_s4=0.4)
check_cmdstan_toolchain(fix=T)
set_cmdstan_path()
mod <- cmdstan_model('feb_sim1.stan', pedantic=T)
fit <- mod$sample(data=data, chains=3, parallel_chains=3, iter_sampling=1000, refresh=10, iter_warmup=2000, save_warmup=FALSE, 
                  init=list(inits_chain1,inits_chain2,inits_chain3))
...
traceplot1
traceplot2 <- traceplot(stanfit, pars=c('a','Tmin','Tmax', 'pv1_s1', 'pv1_s2', 'pv1_s3' ,'pv1_s4'))
traceplot2
...
fit1 <- as.data.frame(summary(stanfit,pars=c('a','Tmin','Tmax','pv1_s1', 'pv1_s2', 'pv1_s3', 'pv1_s4'), probs = c(0.025, 0.975))$summary)
fit1[,"real_value"] <- c(a,Tmin,Tmax, pv1_s1, pv1_s2, pv1_s3, pv1_s4)
fit1 <- data.frame(parameter=c('a','Tmin','Tmax','pv1_s1', 'pv1_s2', 'pv1_s3', 'pv1_s4'), real_value=fit1$real_value, mean=fit1$mean, lci=fit1$`2.5%`,uci=fit1$`97.5%`)
...
fit2[,"real_value"] <- c(su[29], r[29], 
                         (e1[29,1]) + (i1[29,1]) + (e2[29,1]) + (i2[29,1]),
                         (e1[29,2]) + (i1[29,2]) + (e2[29,2]) + (i2[29,2]),
                         (e1[29,3]) + (i1[29,3]) + (e2[29,3]) + (i2[29,3]),
                         (e1[29,4]) + (i1[29,4]) + (e2[29,4]) + (i2[29,4]),
                         moP[29,1], moP[29,2], moP[29,3], moP[29,4], 
                         moS[29,1], moS[29,2], moS[29,3], moS[29,4])
fit2 <- data.frame(initial_state=c('S','R','V_s1','V_s2','V_s3','V_s4','moP1','moP2','moP3','moP4','moS1','moS2','moS3','moS4'), 
...
fit2[15,] <- fit2[3,]+fit2[7,]+fit2[11,]
fit2[16,] <- fit2[4,]+fit2[8,]+fit2[12,]
fit2[17,] <- fit2[5,]+fit2[9,]+fit2[13,]
fit2[18,] <- fit2[6,]+fit2[10,]+fit2[14,]
fit3 <- fit2[15:18,]
fit3[,"real_value"] <- c((e1[29,1]+i1[29,1]+moP[29,1]+moS[29,1]+e2[29,1]+i2[29,1]), 
                         (e1[29,2]+i1[29,2]+moP[29,2]+moS[29,2]+e2[29,2]+i2[29,2]),
                         (e1[29,3]+i1[29,3]+moP[29,3]+moS[29,3]+e2[29,3]+i2[29,3]), 
                         (e1[29,4]+i1[29,4]+moP[29,4]+moS[29,4]+e2[29,4]+i2[29,4]))
fit3[,"initial_state"] <- c(c('I_1','I_2','I_3','I_4'))
fit3 <- data.frame(initial_state=c('I_1','I_2','I_3','I_4'), 
