...
inits_chain1 <- list(log_rho=-1.5, init=ii, log_phiNB=0.3, log_B=matrix(0.05,ncol=4,nrow=data$Nt),delta=0.1,b=0.5)  
inits_chain2 <- list(log_rho=-2, init=ii, log_phiNB=1, log_B=matrix(0.2,ncol=4,nrow=data$Nt), delta=0.3,b=0.4)
inits_chain3 <- list(log_rho=-2.2, init=ii, log_phiNB=0.5, log_B=matrix(0.1,ncol=4,nrow=data$Nt), delta=0.2,b=0.3)

# fit model
check_cmdstan_toolchain(fix=T)
set_cmdstan_path()
mod <- cmdstan_model('with_seroprevalance.stan', pedantic=T)
